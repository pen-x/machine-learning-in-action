...
def clipAlpha(aj, H, L):
    """ Filter alpha with upper and lower boundary. """
    if aj > H:
        aj = H
    if L > aj:
        aj = L
...
def smoSimple(dataMatIn, classLabels, C, toler, maxIter):
    ...
    iter = 0    # stores number of dataset loop that doesn't change alphas continuely
    while iter < maxIter:
        alphaPairsChanged = 0
        for i in range(m):  # dataset loop
            ...
            if ((labelMat[i] * Ei < - toler) and (alphas[i] < C)) or ((labelMat[i] * Ei > toler) and (alphas[i] > 0)):
                ...
                if L == H:
                    print('L == H')
                    continue
                eta = 2.0 * dataMat[i, :] * dataMat[j, :].T - dataMat[i, :] * dataMat[i, :].T - dataMat[j, :] * dataMat[j, :].T     # K11 + K22 - 2K12
                if eta >= 0:
                    print('eta >= 0')
                    continue
...
def testDigits(kTup=('rbf', 10)):
    ...
    dataArr, labelArr = loadDataSet('data/digits/testDigits')
    errorCount = 0
    dataMat = mat(dataArr); labelMat = mat(labelArr).transpose()
    m, n = shape(dataMat)
    for i in range(m):
...
if __name__ == '__main__':
    # dataArr, labelArr = loadDataSet('./data/testSet.6.txt')
    # b, alphas = smoSimple(dataArr, labelArr, 0.6, 0.001, 40)
    # print(b); print(alphas[alphas > 0])

    # dataArr, labelArr = loadDataSet('./data/testSet.6.txt')
    # b, alphas = smoP(dataArr, labelArr, 0.6, 0.001, 40)
    # print(b); print(alphas[alphas > 0])
    # w = calcWs(alphas, dataArr, labelArr); print(w)
